...
class Smarty_Internal_Compile_Section extends Smarty_Internal_Compile_Private_ForeachSection
{
     ...
     * @var array
     */
    public $nameProperties = array('first', 'last', 'home', 'iteration', 'show', 'total', 'rownum', 'index_prev',
                                   'index_next', 'loop');
    ...
    public function compile($args, Smarty_Internal_TemplateCompilerBase $compiler)
    {
        ...
        $incFor = array();
        $cmpFor = array();
        $propValue = array('home' => "{$sectionVar}->value['home']", 'show' => 'true', 'step' => 1,
                           'iteration' => "{$local}iteration",

        );
        $propType = array('home' => 2, 'iteration' => 2, 'show' => 0, 'step' => 0,);
        ...
        if ($propType[ 'step' ] == 0) {
            if ($propValue[ 'step' ] == 1) {
                $incFor[ 'home' ] = "{$sectionVar}->value['home']++";
            } elseif ($propValue[ 'step' ] > 1) {
                $incFor[ 'home' ] = "{$sectionVar}->value['home'] += {$propValue['step']}";
            } else {
                $incFor[ 'home' ] = "{$sectionVar}->value['home'] -= " . - $propValue[ 'step' ];
            }
        } else {
        ...
        $initFor[ 'home' ] = "{$sectionVar}->value['home'] = {$propValue['start']}";
        ...
        if (isset($namedAttr[ 'index_prev' ])) {
            $output .= "{$sectionVar}->value['index_prev'] = {$propValue['home']} - {$propValue['step']};\n";
        }
        if (isset($namedAttr[ 'index_next' ])) {
            $output .= "{$sectionVar}->value['index_next'] = {$propValue['home']} + {$propValue['step']};\n";
        }
